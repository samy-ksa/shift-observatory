...
// Label positions within SVG viewBox (0 0 730 600)
const LABEL_POS: Record<string, { x: number; y: number }> = {
  "Riyadh": { x: 400, y: 370 },
  "Makkah": { x: 210, y: 400 },
  "Madinah": { x: 195, y: 270 },
  "Eastern Province": { x: 510, y: 310 },
  "Al Qassim": { x: 320, y: 255 },
  "Ha'il": { x: 265, y: 195 },
  "Tabuk": { x: 105, y: 220 },
  "Northern Borders": { x: 320, y: 115 },
  "Jazan": { x: 255, y: 565 },
  "Najran": { x: 365, y: 535 },
  "Al Baha": { x: 240, y: 465 },
  "Al Jouf": { x: 105, y: 115 },
...
// Giga-project positions in SVG coordinates
const GIGA_PROJECTS = [
  { name: "NEOM", x: 75, y: 228 },
  { name: "Red Sea", x: 118, y: 310 },
  { name: "AMAALA", x: 95, y: 272 },
  { name: "Qiddiya", x: 375, y: 375 },
  { name: "Diriyah", x: 388, y: 358 },
  { name: "New Murabba", x: 398, y: 348 },
  { name: "Jeddah Central", x: 158, y: 412 },
];

// Color scale: low risk (green) → high risk (orange/red)
function getRiskColor(score: number): string {
  if (!score || score === 0) return "#1E293B";
  const t = Math.max(0, Math.min(1, (score - 25) / (50 - 25)));
...
interface RegionData {
  ...
  gdp_share?: number;
  top_sectors?: { name: string; risk_label: string }[];
  giga_projects?: string[];
}
...
// MAIN COMPONENT
// ============================================================
export default function KSAMap({ regions, selectedRegion, onSelectRegion, lang = "en", riskLabels, legendLabels }: KSAMapProps) {
  const [hovered, setHovered] = useState<string | null>(null);
  const [tooltipPos, setTooltipPos] = useState({ x: 0, y: 0 });

  const regionMap = useMemo(() => {
    const map = new Map<string, RegionData>();
    regions.forEach((r) => map.set(r.name_en, r));
    return map;
  }, [regions]);
    ...
    <div className="relative" onMouseMove={handleMouseMove}>
      ...
      >
        ...
        {KSA_REGIONS.map((region) => {
          const dataName = SVG_TO_DATA[region.name];
          const data = dataName ? regionMap.get(dataName) : null;
          const isSelected = selectedRegion === dataName;
          const isHovered = hovered === dataName;
          const score = data?.ai_risk_score ?? 0;
          ...
          return (
            <path
              ...
              opacity={hovered && !isHovered && !isSelected ? 0.45 : 1}
              className="cursor-pointer"
              style={{ transition: "opacity 0.2s, stroke 0.2s, fill 0.2s" }}
              onMouseEnter={() => setHovered(dataName ?? null)}
              onMouseLeave={() => setHovered(null)}
              onClick={() => dataName && onSelectRegion(dataName)}
            />
          );
        })}

        {/* Labels */}
        {Object.entries(LABEL_POS).map(([name, pos]) => {
          const data = regionMap.get(name);
          if (!data) return null;
          const isLarge = data.total > 1_000_000;
          const isMedium = data.total > 200_000;
          ...
          return (
            <g key={name} className="pointer-events-none">
              <text
                ...
                fill="white"
                fontSize={isLarge ? 13 : isMedium ? 10 : 8}
                fontWeight={selectedRegion === name ? 700 : 500}
                style={{
                  textShadow:
                    "0 1px 3px rgba(0,0,0,0.95), 0 0 6px rgba(0,0,0,0.8), 1px 1px 2px rgba(0,0,0,0.9)",
                  letterSpacing: "0.02em",
                }}
              >
                {lang === "ar" && data.name_ar ? data.name_ar : name}
              </text>
              {isLarge && (
                <text
                  x={pos.x}
      ...
      {/* Hover tooltip */}
      {hovered && (() => {
        const data = regionMap.get(hovered);
        if (!data) return null;
        return (
          <div
            className="fixed z-50 pointer-events-none bg-[#0F172A]/95 border border-cyan-500/20 rounded-lg px-4 py-3 shadow-2xl backdrop-blur-sm"
            style={{
          ...
          >
            ...
            <div className="flex justify-between text-xs">
              <span className="text-gray-400">{lang === "ar" ? "% سعوديون" : "Saudi %"}</span>
              <span className="text-white font-mono">{data.pct_saudi}%</span>
            </div>
            <div className="flex justify-between text-xs mt-1">
              <span className="text-gray-400">{lang === "ar" ? "مخاطر الذكاء الاصطناعي" : "AI Risk"}</span>
              <span
                className="font-mono font-bold"
                style={{ color: getRiskColor(data.ai_risk_score) }}
              >
                {data.ai_risk_score}{" "}
                <span className="text-[10px] font-normal">
                  {riskLabels ? (riskLabels[data.ai_risk_score >= 45 ? "high" : data.ai_risk_score >= 38 ? "moderate" : data.ai_risk_score >= 30 ? "low" : "very_low"] || getRiskLabel(data.ai_risk_score)) : getRiskLabel(data.ai_risk_score)}
                </span>
              </span>
            </div>
            <div className="text-[10px] text-gray-600 mt-2 text-center">
              {lang === "ar" ? "انقر للتفاصيل" : "Click for details"}
            </div>
          </div>
        );
      })()}
      ...
      <div className="flex items-center justify-center gap-6 mt-4 text-xs text-gray-400">
        <div className="flex items-center gap-2">
          <div
            ...
            style={{ background: getRiskColor(28) }}
          />
          <span>{legendLabels?.lower ?? "Lower risk"}</span>
        </div>
        <div className="flex items-center gap-1">
          <div
            className="w-4 h-3 rounded-sm"
            style={{ background: getRiskColor(36) }}
          />
          <span>→</span>
          <div
            className="w-4 h-3 rounded-sm"
            style={{ background: getRiskColor(45) }}
          />
          <span>{legendLabels?.higher ?? "Higher risk"}</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full bg-[#D4AF37] animate-pulse" />
          <span>{legendLabels?.giga ?? "Giga-project"}</span>
        </div>
      </div>
    </div>
  );
